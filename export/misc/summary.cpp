...
void Summary::printContent() {
    ...
    while (query3.next()) {
        ...
        painter.drawText(QRectF(pr.width()/2, yco, pr.width()-pr.x()-pr.x(), QFontMetricsF(painter.font()).height()),_global::wkBez(this->event, query3.value(0).toString()),QTextOption(Qt::AlignVCenter));
        QSqlQuery query4;

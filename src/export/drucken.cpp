...
#include "misc/header/urkunde.h"
#include "../global/header/settings.h"

int Drucken::detailinfo = 0;
int Drucken::coverID = 0;

QPrinter::PaperSize Drucken::paperSize = QPrinter::A4;
QPrinter::Orientation Drucken::orientation = QPrinter::Portrait;

int Drucken::headFootID = 0;

Drucken::Drucken() : QThread() {
    showPreview = false;
    selectClub = false;
    selectTN = false;
    selectWK = false;
...
void Drucken::setOutputType(int set) {
    outputType = set;
}

int Drucken::getOutputType() {
    return outputType;
}

void Drucken::setDetailInfo(int set) {
    detailinfo = set;
}

void Drucken::setShowPreview(bool set) {
    showPreview = set;
}

void Drucken::setSelectClub(bool set) {
    selectClub = set;
}

void Drucken::setSelectRiege(bool set) {
    selectRiege = set;
}

void Drucken::setSelectTN(bool set) {
    selectTN = set;
}

void Drucken::setSelectWK(bool set) {
    selectWK = set;
}

void Drucken::setSelectDis(bool set) {
    selectDis = set;
}

void Drucken::setSelectDetail(bool set) {
    selectDetail = set;
}

void Drucken::setPaperSize(QPrinter::PaperSize set) {
    paperSize = set;
}

QPrinter::PaperSize Drucken::getPaperSize() {
    return paperSize;
}

void Drucken::setOrientation(QPrinter::Orientation set) {
    orientation = set;
}

QPrinter::Orientation Drucken::getOrientation() {
    return orientation;
}

void Drucken::setHeadFootID(int set) {
    headFootID = set;
}

void Drucken::setCoverID(int set) {
    coverID = set;
}

void Drucken::setOutputFileName(QString set) {
    outputFileName = set;
}

void Drucken::setWKNumber(QString set) {
    singleWK = set;
}

void Drucken::setVerein(int set) {
    QList<int> lst;
    lst.append(set);
    vereinNumbers = lst;
}

void Drucken::setDetailQuery(QString set) {
    detailQuery = set;
}

void Drucken::setSelectedClubs(QList<int> set) {
    selectedClubs = set;
}

void Drucken::setSelectedDisziplinen(QList<QList<int> > set) {
    selectedDisziplinen = set;
}

void Drucken::setSelectedRiegen(QStringList set) {
    selectedRiegen = set;
}

void Drucken::setSelectedTN(QList<int> set) {
    selectedTN = set;
}

void Drucken::setSelectedTNWK(QString set) {
    selectedTNWK = set;
}

void Drucken::setSelectedWKs(QStringList set) {
    selectedWKs = set;
}

void Drucken::setTeilnehmerNumbers(QList<int> set) {
    teilnehmerNumbers = set;
}

void Drucken::setFinish(bool set) {
    finish = set;
}

void Drucken::printHeadFoot() {
...
void Drucken::printDescriptor(QString swknr, bool f) {
    QSqlQuery query;
    ...
    painter.drawText(QRectF(pr.x(), yco-mmToPixel(2.6), pr.width()-pr.x()-pr.x(), (QFontMetricsF(painter.font()).height())*2.3),"WK Nr. " + query.value(0).toString() + " " + query.value(1).toString() + _global::wkBez(swknr) + fort,QTextOption(Qt::AlignVCenter));
    yco += QFontMetricsF(painter.font()).height()*2.3;
    printSubHeader();
}

void Drucken::printContent() {}

void Drucken::printSubHeader() {}

void Drucken::newPage(bool print) {
    curr_printer->newPage();
    yco = top_yco;
    if (print) printHeadFoot();
}

void Drucken::finishPrint() {
    painter.end();
    customImages.clear();
}
...
void Drucken::run() {
    ...
    if (outputType != 2) {
        ...
        vereineQuery.bindValue(0,_global::checkHWK());
        vereineQuery.exec();
    ...
    wkQuery.bindValue(0,_global::checkHWK());
    wkQuery.exec();
    wkNumbers.clear();
    wkWahl.clear();
    while (wkQuery.next()) {
    ...
    riegenQuery.bindValue(0,_global::checkHWK());
    riegenQuery.exec();
    riegenNumbers.clear();
    ...
    if (selectTN) {
        emit requestTN();
        if (finish) return;
        if (_global::checkTyp(selectedTNWK)==1) {
            ...
            teamTeilnehmer.bindValue(0,_global::getRunde());
            teamTeilnehmer.exec();
    ...
    disziplinenQuery.bindValue(0,_global::checkHWK());
...
void Drucken::setPrinterFont(int size, bool bold, bool italic) {
...
int Drucken::mmToPixel(double mm) {
    int px = ((double)curr_printer->width() * mm / ((double)curr_printer->widthMM()+10.0));
    return px;
}

void Drucken::drawStandardRow(QString plst, QString name, QString jg, QString verein, QString points, QString extra) {
...
QString Drucken::readDetailInfo(bool head, QString verein) {
    QString detail;
    if (head) {
        switch (detailinfo) {
...
void Drucken::setTypeString(QString t) {
    typeString = t;
}

void Drucken::printCustomPage(int mode, int layoutid, QStringList tndata, QString tnwk) {
    QFont font2(font);
    QSqlQuery query2;
    if (tnwk == "") {
        ...
        if (layoutData.value(0).toInt() == 0) {
            ...
            case 6 : {
                    if (tndata.size()>0) {
                        ...
                        Urkunde *urkunde = qobject_cast<Urkunde*>(this);
                        if (urkunde != 0) isUrkunde=true;
                        if (isUrkunde && urkunde->getRundenErgebnisse() == true) {
            ...
            case 8 : field = query2.value(5).toString() + _global::wkBez(query2.value(6).toString()); break;
            ...
            case 14: {
                    if (tndata.size()>0) {
                        if (_global::checkTyp(tnwk) == 1 || _global::checkTyp(tnwk) == 2) {
                            QSqlQuery teamq;
                            if (_global::checkTyp(tnwk) == 1) {
...
QStringList Drucken::readDetailInfos(QString verein) {
    if (verein.right(2) == "M.") verein = verein.left(verein.length()-8);
...
void Drucken::drawHighlightRect(qreal y, qreal h) {
...
void Drucken::drawTextLine(QString text, int x, bool newLine) {
    if (x == 0) {
        x = pr.x();
    }

#include "header/medallienspiegel.h"
#include "../../global/header/_global.h"
#include "../../global/header/result_calc.h"

void Medallienspiegel::print(QPrinter *printer) {
    ids.clear();
    gesamt.clear();
    gold.clear();
    silber.clear();
    ...
    for (int i=0;i<wkNumbers.size();i++) {
        ...
        QList<QStringList> rlist = Result_Calc::resultArrayNew(currWK,_global::checkTyp(currWK));
        for(int j=0;j<rlist.size();j++) {
            ...
            } else if (_global::checkTyp(currWK) == 1) {
...
void Medallienspiegel::printContent() {
    printSubHeader();
    for (int i=0;i<ids.size();i++) {
...
void Medallienspiegel::printSubHeader() {

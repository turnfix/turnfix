#include <math.h>
#include <QDate>
#include "header/ausschreibung.h"
#include "../../global/header/_global.h"

void Ausschreibung::print(QPrinter *printer) {
...
void Ausschreibung::printContent() {
    ...
    while (query3.next()) {
        ...
        painter.drawText(QRectF(pr.width()/2, yco, pr.width()-pr.x()-pr.x(), QFontMetricsF(painter.font()).height()),_global::wkBez(query3.value(1).toString()),QTextOption(Qt::AlignVCenter));
        yco += mmToPixel(1.3);
        int i=0;
...
void Ausschreibung::drawHeader(QString text) {
...
void Ausschreibung::drawUserText(QString text, QString header) {
    double plus;
    QTextOption op;
...
void Ausschreibung::checkNewPage(double plus) {
    if (plus > max_yco) {
        newPage();
    }
}

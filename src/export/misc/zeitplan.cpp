#include <QTime>
#include "header/zeitplan.h"
#include "../../global/header/_global.h"

void Zeitplan::print(QPrinter *printer) {
...
void Zeitplan::printContent() {
    ...
    while (query.next()) {
        ...
        setPrinterFont(11);
        painter.drawText(QRectF(pr.x()+(query.value(1).toInt()-1)*((pr.width()-pr.x()-pr.x())/3), yco, ((pr.width()-pr.x()-pr.x())/3), QFontMetricsF(painter.font()).height()),"Wk: " + query.value(2).toString() + " " + query.value(5).toString() + " " + _global::wkBez(query.value(2).toString()),QTextOption(Qt::AlignVCenter | Qt::AlignLeft));
        yco += mmToPixel(4.2);
        max_y = yco;
    }
    finishPrint();

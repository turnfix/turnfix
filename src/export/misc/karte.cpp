...
void Karte::print(QPrinter *printer) {
...
void Karte::printContent() {
    ...
    while (query.next()) {
        ...
        if (_global::checkTyp(query.value(0).toString()) != 2) painter.drawText(QRectF(pr.width()/2-mmToPixel(7.5), yco, (pr.width()-pr.x()-pr.x())/2, QFontMetricsF(painter.font()).height()),"Jahrgang: " + query.value(3).toString(),QTextOption(Qt::AlignVCenter));
        yco += mmToPixel(6.6);
        ...
        while (query2.next()) {
            for (int kp=0;kp<=query2.value(3).toInt();kp++) {
                ...
                checkKuer.bindValue(0,_global::checkHWK());
        ...
        int rows = 0;
        if (_global::checkTyp(query.value(0).toString()) == 2) {
            setPrinterFont(8);
            QSqlQuery teamq2;
            QString teamstr;

...
int QKartenTableModel::columnCount(const QModelIndex &) const {
    return 2;
}

QVariant QKartenTableModel::data(const QModelIndex &index, int role) const {
    if (!index.isValid())
        return QVariant();
...
QVariant QKartenTableModel::headerData(int section, Qt::Orientation orientation, int role) const {
    if (role != Qt::DisplayRole)
        return QVariant();
...
Qt::ItemFlags QKartenTableModel::flags(const QModelIndex &index) const  {
    if (!index.isValid())
...
bool QKartenTableModel::setData(const QModelIndex &index, const QVariant &value, int role) {

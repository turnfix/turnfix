...
#include "header/dlg_save_layout.h"
#include "ui_dlg_save_layout.h"

Save_Layout::Save_Layout(QWidget *parent) : QDialog(parent), ui(new Ui::Save_Layout) {
    ui->setupUi(this);
...
void Save_Layout::layoutSelectionChange() {
...
void Save_Layout::setItems(QList<EditorTextItem*> itms) {
    items = itms;
}

QList<EditorTextItem*> Save_Layout::getItems() {
    return items;
}

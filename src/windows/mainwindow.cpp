...
void MainWindow::initTabs() {
    ...
    _global::setWkVars(_global::getWkNr(), runde, hwk);
...
void MainWindow::showWKConf() {
    Event_Dialog *nwkw = new Event_Dialog(_global::getWkNr(),this);
    nwkw->exec();
}
...
void MainWindow::newNumbers() {
    ...
    query4.bindValue( 0, _global::checkHWK());
    query4.exec();
    while (query4.next()) {
        QSqlQuery query3;
...
void MainWindow::editPass() {
    Pass_Dialog *pass = new Pass_Dialog(this);
    pass->exec();
}
...
void MainWindow::sendMLists() {
    Mail_Dialog *vn = new Mail_Dialog(this);

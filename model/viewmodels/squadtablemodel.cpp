...
#include <QColor>
#include <math.h>
#include "squadtablemodel.h"
#include "libs/fparser/fparser.hh"
#include "model/objects/event.h"
#include "src/global/header/_global.h"

SquadTableModel::SquadTableModel(Event *event, QObject *parent) : QAbstractTableModel(parent) {
    geraet=-1;
    this->event = event;
}

int SquadTableModel::rowCount(const QModelIndex &) const {
    return starter.size()*versuche;
}

int SquadTableModel::columnCount(const QModelIndex &) const {
    return 5+extraColumns.size();
}

QVariant SquadTableModel::data(const QModelIndex &index, int role) const {
    if (!index.isValid())
        return QVariant();
...
QVariant SquadTableModel::headerData(int section, Qt::Orientation orientation, int role) const {
    if (role != Qt::DisplayRole)
        return QVariant();
...
Qt::ItemFlags SquadTableModel::flags(const QModelIndex &index) const  {
    if (!index.isValid())
...
bool SquadTableModel::setData(const QModelIndex &index, const QVariant &value, int role) {
...
void SquadTableModel::setTableData(QString rg, int g, int v, bool k, bool j) {
    beginResetModel();
...
QList<int> SquadTableModel::getExtraColumnIDs() {
    return extraColumns;
}

int SquadTableModel::getCurrentID(const QModelIndex &index) {
    return starter.at(static_cast<int>(floor(index.row()/versuche))).at(4).toInt();
}

int SquadTableModel::getNextID(const QModelIndex &index) {
...
int SquadTableModel::getLastID(const QModelIndex &index) {
